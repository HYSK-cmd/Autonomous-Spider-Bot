...
class SpiderEnv(gym.Env):
    ...
    def __init__(self, render_mode="human"):
        ...
        # Same setup as original, with episode/goal controls for shaping.
        self.goal_x = 5.0
        self.max_steps = 1000
    ...
    def step(self, action):
        ...
        old_goal_dist = np.linalg.norm(np.array([self.goal_x - old_position[0], -old_position[1]]))
        ...
        progress_x = new_position[0] - old_position[0]
        new_goal_dist = np.linalg.norm(np.array([self.goal_x - new_position[0], -new_position[1]]))

        lidar = self._get_lidar()
        front_lidar = np.concatenate([lidar[-20:], lidar[:21]])
        front_min = np.min(front_lidar)

        reward = 3.0 * progress_x
        reward += 2.0 * (old_goal_dist - new_goal_dist)

        if front_min > 1.5:
            reward += 0.3 * max(0.0, vx)
        elif vx > 0:
            reward -= 0.8 * max(0.0, vx) * (1.5 - front_min)
            reward += 0.1 * (abs(vy) + abs(wz))

        if new_position[0] >= self.goal_x:
            reward += 40.0
            terminated = True
        elif collision:
            reward -= 25.0
            ...
            if min_distance < safe_distance:
                reward -= ((safe_distance - min_distance) ** 2) * 3.0
            reward -= 0.01

        if self.episode_steps >= self.max_steps and not terminated:
            truncated = True
            reward -= 2.0
